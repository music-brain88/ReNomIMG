...
class WrapperYoloDarknet(Wrapper):
    ...
    def get_bbox(self, model_original_formatted_out):
        ...
        yolo_format_out = model_original_formatted_out.reshape(N, cell, cell, bbox*5+self._num_class)
        ...
        for i in range(len(result[0])):
            obj_in_one_image[indexes[0][i]].append({
                "class": indexes[2][i],
                "box": box[indexes[0][i], indexes[1][b]],
                "score": probs[indexes[0][i], indexes[1][b], indexes[2][b]]
            })
        return result
